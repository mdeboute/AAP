...
//}
 ...
 //    double np = std::tgamma(n + 1) / (std::tgamma(k + 1) * std::tgamma(n - k + 1));
 //    std::cout << "np equals " << np << std::endl;
 //    for (int i = 0; i < np; ++i)
 //    {
 //        next(partition);
 //        partitions.push_back(partition);
...
void new_generate_partitions(int n, int k, int i, int nb_selected, std::vector<bool> & partition, std::vector<std::vector<bool>> & partitions)
 {
     ...
     new_generate_partitions(n, k, i + 1, nb_selected + 1, partition, partitions);
     partition.pop_back();
     partition.push_back(0);
     new_generate_partitions(n, k, i + 1, nb_selected, partition, partitions);
     partition.pop_back();
     return;
 }
 ...
 std::vector<FighterVertex> better_bruteforce_solve(const Graph &graph)
 {
     ...
     while (solFound == 0)
     {
         ...
         new_generate_partitions(n, lowerBound, 0, 0, partition, partitions);
